...
public class CommandDecreasePixel implements ICommand {
    ...
    public boolean execute(Canvas canvas) {
        ...
        char decrementedChar = (char) (canvas.getPixel(this.x, this.y) - 1);
//        System.out.println(canvas.getPixel(this.x, this.y));
//        System.out.println(decrementedChar);

        if (decrementedChar > 31 && decrementedChar < 127) {
            this.previousCharacter = canvas.getPixel(this.x, this.y);
            canvas.decreasePixel(this.x, this.y);
            this.character = decrementedChar;
            isExecuted = true;
            return true;
        }
        isExecuted = false;
